      ...
      <div class="form-box">
        ...
        <div class="form-group">
          ...
          <div class="form-item item-wrap">
            <label class="form-tit">银行预留手机号</label>
            <div class="form-ipt">
              <input
                type="tel"
                maxlength="11"
                class="ipt"
                v-model="bankInfo.userPhone"
  ...
  methods: {
    ...
    async checkBindBankStatus() {
      ...
      if (resultinfo == 1) {
        showLoading("请稍等");
        this.$api
          .changeStatusInYunNan({
            applyId: applyInfo.niurunApplyId
          })
          .then(res => {
    ...
    async submitForm() {
      ...
      if (
        bankInfo.status != 0 ||
        (bankInfo.status == 0 && this.checkBankInfoIsUpdate())
      ) {
        showLoading("提交中");
        res = await this.$api.saveBankInfoInYunNan({
          applyId: this.applyInfo.niurunApplyId,
