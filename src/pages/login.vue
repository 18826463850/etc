    ...
    <div class="main">
      <div class="form-box">
        <div class="from-brick" v-if="mode==1">
          ...
          <div class="form-group">
            <div class="form-item">
              ...
              <div class="form-ipt">
                <input type="tel" placeholder="请输入您的手机号" v-model="mobile" @blur="onBlur" @focus="onFocus" />
              </div>
            </div>
            <div class="form-item item-code">
              <div class="ui-icon icon-code">
              </div>
              <div class="form-ipt">
                <input type="tel" placeholder="请输入您的验证码" v-model="validCode"  @blur="onBlur" @focus="onFocus" />
              </div>
          ...
          <van-button type="primary" :loading="doing" class="btn-default" :disabled="btnDisabled" @click="login" >登 录
          </van-button>

       
          <div class="login-action one">
              <div class="act" @click="loginSwitch(2)">
                  密码登录
                </div>
          </div>
        ...
        <div class="from-brick" v-if="mode==3">
          ...
          <van-button type="primary" :loading="doing" class="btn-default" :disabled="btnDisabled" @click="login" >绑定手机号</van-button>
          <div class="login-action one">
              <div class="act" @click="loginSwitch(1)">
                  免密登录 
               </div>
          </div>
...
<script>
  ...
  import { tip, checkMobile, showLoading, hideLoading, checkValidCode,isInWeiXin,delUrlParams} from "@/utils"
  ...
  export default {
    ...
    computed: {
      ...
      btnDisabled() {
        ...
        if (mode == 1 || mode == 3) {
          disabled = !(checkMobile(this.mobile) && checkValidCode(this.validCode));
        } else if (mode == 2) {
    ...
    methods: {
      ...
      async login() {
        ...
        if (data && data.code == 200 && data.data && data.data.token) {
          ...
          tip(data && data.message || (this.mode == 3 ? "绑定失败": "登录失败"));
